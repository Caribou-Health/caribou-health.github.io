 ...
 * clinician recommendations. It does not provide medical advice.
 */
...
const ConditionsDatabase = [
  ...
  { id: 'obesity', name: 'Obesity Management', aliases: ['weight management', 'weight loss', 'bariatric', 'metabolic syndrome'] },
...
const ClinicalDatabase = {
  conditions: {
    'knee-sprain': {
      ...
      clinicalInfo: {
        ...
        prevalence: 'Knee sprains account for approximately 40% of all sports-related injuries. ACL injuries occur in approximately 200,000 individuals annually in the United States.',
        ...
        reference: 'American Academy of Orthopaedic Surgeons (AAOS); Journal of Athletic Training'
      },
      nutritionGuidelines: [
    ...
    'ankle-sprain': {
      ...
      clinicalInfo: {
        ...
        prevalence: 'Ankle sprains are the most common musculoskeletal injury, with approximately 2 million occurring annually in the US. They account for 10-30% of sports injuries.',
        ...
        reference: 'British Journal of Sports Medicine; American College of Foot and Ankle Surgeons'
      },
      nutritionGuidelines: [
    ...
    'back-pain': {
      ...
      clinicalInfo: {
        ...
        prevalence: 'LBP affects up to 84% of adults at some point in their lives. It is the leading cause of disability worldwide and the most common reason for missed work days.',
        ...
        reference: 'The Lancet Series on Low Back Pain; American Physical Therapy Association Clinical Practice Guidelines'
      },
      nutritionGuidelines: [
    ...
    'shoulder-injury': {
      ...
      clinicalInfo: {
        ...
        prevalence: 'Shoulder pain affects 18-26% of adults at any given time. Rotator cuff disorders account for up to 70% of shoulder pain cases. Incidence increases with age, particularly after 40.',
        ...
        reference: 'Journal of Shoulder and Elbow Surgery; American Academy of Orthopaedic Surgeons'
      },
      nutritionGuidelines: [
    ...
    'diabetes-type2': {
      ...
      clinicalInfo: {
        ...
        prevalence: 'T2DM affects approximately 462 million people worldwide (6.3% of the global population). Prevalence has more than doubled since 1980 and continues to rise with increasing obesity rates.',
        prognosis: 'With proper management including lifestyle modifications, medication adherence, and regular monitoring, individuals with T2DM can achieve good glycemic control and reduce complication risk. HbA1c targets typically range from 6.5-7.0%.',
        reference: 'American Diabetes Association Standards of Care; The Lancet Diabetes & Endocrinology'
      },
      nutritionGuidelines: [
  ...
  getConditionData(diagnosisId) {
    ...
    if (this.conditions[diagnosisId]) {
      return this.conditions[diagnosisId];
    }

    // If not, look up the condition name from CONDITION_MAPPINGS and create a basic entry
    const mapping = CONDITION_MAPPINGS.find(c => c.id === diagnosisId);
    if (mapping) {
      return {
        name: mapping.name,
        ...
        exercises: this.conditions['other'].exercises,
        ptDuration: 20,
        recoveryWeeks: 8
      };
    }
...
const MealDatabase = {
  ...
  lunch: [
    ...
    },
    {
      ...
      tags: ['gluten-free', 'dairy-free'],
      recipe: {
        ingredients: ['8 oz salmon fillet', '6 cups mixed greens', '1 cucumber, sliced', '4 tbsp olive oil', '2 tbsp lemon juice', 'Dill and salt to taste'],
  ...
  dinner: [
    {
      ...
      tags: ['gluten-free', 'dairy-free'],
      recipe: {
        ingredients: ['12 oz salmon fillet', '2 cups brown rice', '4 cups mixed vegetables', '3 tbsp olive oil', 'Fresh herbs (dill, parsley)', 'Lemon wedges'],
    ...
    },
    {
      ...
      tags: ['dairy-free'],
      recipe: {
        ingredients: ['1 lb chicken breast, sliced', '4 cups stir-fry vegetables', '2 cups brown rice', '4 tbsp low-sodium soy sauce', '2 tbsp sesame oil', 'Ginger and garlic'],
    ...
    },
    {
      ...
      tags: ['vegan', 'gluten-free'],
      recipe: {
        ingredients: ['4 cups mixed vegetables', '2 cans chickpeas, drained', '2 cans coconut milk', '4 tbsp curry paste', '2 cups basmati rice', 'Fresh cilantro'],
...
function navigateTo(page) {
  ...
  if (targetPage) {
    ...
    if (page === 'nutrition') updateNutritionPage();
    if (page === 'medication') updateMedicationPage();
    if (page === 'physicaltherapy') updatePTPage();
    if (page === 'condition') updateConditionPage();
  ...
  if (combinedExercises.length > 0) {
    ...
    conditionOverview += `
      <div style="display: flex; align-items: center; gap: var(--space-2); font-size: 0.85rem;">
        ...
        <span><strong>Physical Therapy:</strong> ${combinedExercises.length} exercises, ~${totalDuration} mins daily</span>
      </div>`;
  }
...
function generateTaskCategories() {
  ...
  // Physical Therapy Category - now uses combined exercises from all conditions
  const combinedExercises = getCombinedExercises();
  if (combinedExercises.length > 0) {
  ...
  // Add additional wellness activities based on condition if time permits
  const primaryCondition = ClinicalDatabase.getConditionData(AppState.userData.diagnosis);
  if (primaryCondition && primaryCondition.wellnessActivities) {
    for (const activity of primaryCondition.wellnessActivities) {
      const activityMinutes = parseDurationToMinutes(activity.duration || '15 minutes');
      if (wellnessMinutesUsed + activityMinutes <= MAX_WELLNESS_MINUTES_PER_DAY) {
        wellnessTasks.push({
        ...
        wellnessMinutesUsed += activityMinutes;
      }
    }
  }
  ...
  if (condition.exercises.length > 0) {
    html += `
      <div class="day-preview-section">
        <h4><span class="dot pt"></span> Physical Therapy</h4>
...
function saveDailyCheckin() {
  ...
  // Update the card to show it's been completed
...
function loadDailyCheckin() {
  ...
  if (AppState.dailyCheckin?.date === today) {
    // Restore selections
    if (AppState.dailyCheckin.pain) {
      selectDailyCheckin('pain', AppState.dailyCheckin.pain);
    }
    if (AppState.dailyCheckin.sleep) {
      selectDailyCheckin('sleep', AppState.dailyCheckin.sleep);
    }
    if (AppState.dailyCheckin.mood) {
      selectDailyCheckin('mood', AppState.dailyCheckin.mood);
    }
...
function updateNutritionPage() {
        ...
        <div class="meal-card-header">
          <div class="meal-icon">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ${icon === 'sunrise' ? '<circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="12" y1="21" x2="12" y2="23"/>' :
                icon === 'sun' ? '<circle cx="12" cy="12" r="5"/><line x1="12" y1="1" x2="12" y2="3"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/>' :
  ...
  if (guidelinesEl && condition.nutritionGuidelines) {
...
const GroceryCategories = {
  'Produce': ['apple', 'banana', 'berries', 'cherry tomatoes', 'tomato', 'cucumber', 'spinach', 'lettuce', 'greens', 'avocado', 'lemon', 'carrot', 'celery', 'onion', 'garlic', 'ginger', 'broccoli', 'vegetables', 'sweet potato', 'fruit', 'cilantro', 'dill', 'parsley', 'herbs'],
  'Proteins': ['chicken', 'turkey', 'salmon', 'steak', 'beef', 'eggs', 'falafel', 'chickpeas', 'lentils'],
  'Dairy & Alternatives': ['yogurt', 'milk', 'plant milk', 'cheese', 'butter', 'cream'],
  'Grains & Bread': ['oats', 'quinoa', 'rice', 'brown rice', 'basmati rice', 'bread', 'wrap', 'pita', 'granola'],
  'Pantry Items': ['olive oil', 'sesame oil', 'honey', 'tahini', 'hummus', 'curry paste', 'soy sauce', 'mustard', 'coconut milk', 'vegetable broth', 'cumin', 'salt', 'pepper', 'cinnamon', 'garlic powder'],
  'Nuts & Seeds': ['almond butter', 'almonds', 'walnuts', 'cashews', 'nuts', 'seeds', 'mixed nuts']
};
...
function generateWeeklyGroceryList() {
  ...
  // Check if it's the first day of the week (relative to firstVisitDate)
  const firstVisit = AppState.firstVisitDate ? new Date(AppState.firstVisitDate) : new Date();
  const today = new Date();
  const daysSinceStart = Math.floor((today - firstVisit) / (1000 * 60 * 60 * 24));
  const currentWeek = Math.floor(daysSinceStart / 7) + 1;
  const dayOfWeek = daysSinceStart % 7;

  // Show grocery list only on day 0 (first day) of each week, or always show for demo purposes
  // For now, we'll show it if it's within first 2 days of the week
  const showGroceryList = dayOfWeek <= 1 || !AppState.firstVisitDate;
  ...
  if (weekLabel) {
    weekLabel.textContent = `Week ${currentWeek} meal plan ingredients`;
  }
  ...
  // Use the initialized meal plan for grocery list
  const allIngredients = [];

  // Collect ingredients from the planned meals
  ...
  allIngredients.forEach(ingredient => {
    ...
    for (const [category, keywords] of Object.entries(GroceryCategories)) {
      if (keywords.some(kw => ingredient.includes(kw))) {
        ...
        const existing = categorizedIngredients[category].find(i =>
          i.name.toLowerCase() === ingredient || ingredient.includes(i.name.toLowerCase())
        );
        if (!existing) {
          categorizedIngredients[category].push({ name: ingredient, checked: false });
        }
        categorized = true;
        break;
      }
    ...
    if (!categorized) {
      const existing = categorizedIngredients['Other'].find(i => i.name === ingredient);
      if (!existing) {
        categorizedIngredients['Other'].push({ name: ingredient, checked: false });
      }
    }
  });
  ...
  for (const [category, items] of Object.entries(categorizedIngredients)) {
    ...
    html += `
      <div class="grocery-category">
        ...
        ${items.map((item, idx) => `
          <div class="grocery-item" data-category="${category}" data-index="${idx}">
            <div class="grocery-item-checkbox" onclick="toggleGroceryItem(this)">
...
// ============================================
// Physical Therapy Page Functions
// ============================================
function updatePTPage() {
  ...
  if (pageTitleEl) {
    if (isViewingFamilyMember) {
      pageTitleEl.innerHTML = `${displayName} Physical Therapy Plan ${familyNote}`;
    } else {
      pageTitleEl.textContent = 'Physical Therapy Plan';
    }
  }
  ...
  if (durationEl) {
    ...
    durationEl.textContent = `~${cappedDuration}`;
  }
  ...
  if (checklist && combinedExercises.length > 0) {
    checklist.innerHTML = combinedExercises.map((exercise, index) => {
      const taskId = `pt-${index}`;
      ...
      return `
        <li class="checklist-item${isCompleted ? ' checked' : ''}" data-task="${taskId}">
          ...
          <div class="checklist-content">
            <div class="checklist-title">
              ${exercise.name}
              ${allConditions.length > 1 ? `<span class="exercise-condition-tag">${exercise.conditionName}</span>` : ''}
            </div>
            <div class="checklist-meta">${exercise.description}</div>
            ${exerciseImage ? `
            ...
            <div class="checklist-time">
              ...
              ${formatTime(exercise.time)} - ${exercise.duration}
            </div>
          </div>
        </li>
      `;
...
function getExerciseImage(exerciseName) {
  ...
  // Exercise illustration database with SVG diagrams
  const exerciseIllustrations = {
    // Stretching exercises
    'morning stretch': generateStretchingSVG('full-body', 'Stand tall, reach arms overhead, hold 15-30 seconds'),
    'gentle stretching': generateStretchingSVG('full-body', 'Slow movements through comfortable range'),
    'light stretching': generateStretchingSVG('full-body', 'Gentle stretches, hold each for 15-20 seconds'),
    'stretching routine': generateStretchingSVG('full-body', 'Complete body stretch sequence'),

    // Walking exercises
    'morning walk': generateWalkingSVG('Maintain steady pace, swing arms naturally'),
    'afternoon activity': generateWalkingSVG('Light walking or movement after meals'),
    'cardiovascular activity': generateCardioSVG('Moderate intensity - you should be able to talk'),

    // Strength exercises
    'strength training': generateStrengthSVG('bodyweight', 'Focus on form over speed'),
    'basic bodyweight': generateStrengthSVG('bodyweight', 'Squats, lunges, push-ups at your level'),

    // Range of motion
    'range of motion': generateROMSVG('Move joints through full comfortable range'),

    // Shoulder exercises
    'pendulum swings': generateShoulderSVG('pendulum', 'Lean forward, let arm swing gently in circles'),
    'wall slides': generateShoulderSVG('wall-slides', 'Back against wall, slide arms up and down'),

    // Breathing exercises
    'deep breathing': generateBreathingSVG('Inhale 4 counts, hold 4, exhale 4'),
    'breathing exercises': generateBreathingSVG('Slow, controlled breaths'),

    // Core exercises
    'core activation': generateCoreSVG('Gentle engagement of abdominal muscles'),

    // Balance exercises
    'balance practice': generateBalanceSVG('Stand near support, practice single-leg stance'),

    // Relaxation
    'evening relaxation': generateRelaxationSVG('Progressive muscle relaxation or meditation'),
    'evening wind-down': generateRelaxationSVG('Gentle stretching and calming activities'),

    // Water exercises
    'water walking': generateWaterSVG('Walk in chest-deep water for low-impact exercise'),

    // Hand exercises
    'hand exercises': generateHandSVG('Make fists, spread fingers, touch fingertips to thumb')
  };

  // Find matching exercise
  for (const [key, svg] of Object.entries(exerciseIllustrations)) {
  ...
  return generateGenericExerciseSVG(exerciseName);
}
...
const sectionTransitionMessages = {
  1: { message: "Great start!", subtitle: "Let's learn about your health habits..." },
  2: { message: "You're doing great!", subtitle: "Now let's talk about exercise..." },
  3: { message: "Almost there!", subtitle: "Just a few nutrition questions..." },
  4: { message: "Last step!", subtitle: "Tell us about your care team..." },
  5: { message: "All done!", subtitle: "Creating your personalized care plan..." }
};
...
function saveStepData(step) {
  switch (step) {
    ...
    case 4:
      AppState.userData.age = document.getElementById('user-age')?.value || '';
      AppState.userData.sex = document.getElementById('user-sex')?.value || '';

      if (AppState.preferredUnits === 'metric') {
        AppState.userData.height = document.getElementById('user-height')?.value || '';
        AppState.userData.weight = document.getElementById('user-weight')?.value || '';
      } else {
        ...
        AppState.userData.height = Math.round((ft * 30.48) + (inches * 2.54));
...
function selectTier(tier) {
  ...
  showCaribouLoading(loadingMessages, () => {
    ...
    // Cura chatbot 24/7 is available for Family tier and above
    // Premium tier gets limited AI personalization
    if (chatbot) {
      if (tier === 'family' || tier === 'enterprise') {
        chatbot.style.display = 'block';
      } else if (tier === 'premium') {
        // Premium gets chatbot but with limited features
        chatbot.style.display = 'block';
      } else {
...
function toggleChatbot() {
  ...
  if (window) {
    ...
    AppState.chatbotOpen = window.classList.contains('open');
...
function sendChatMessage() {
  ...
  setTimeout(() => {
    const response = ChatbotSystem.getResponse(userMessage);
...
function populateIntegratedCarePlan(conditionIds) {
  ...
  // Combine nutrition guidelines from all conditions
  ...
  let planHTML = `
    ...
    <div class="condition-section" style="background: #f0fdf4; padding: var(--space-4); border-radius: var(--radius); margin-bottom: var(--space-4);">
      ...
      <p style="color: var(--gray-600); margin-top: var(--space-2); line-height: 1.6;">
        Your physical therapy program focuses on exercises that help with all your conditions when possible.
        Some exercises may target specific conditions while being gentle on areas affected by others.
        Always communicate with your physical therapist about all your conditions so they can modify exercises appropriately.
      </p>
...
function updateProfilePage() {
  ...
  const fields = {
    ...
    'profile-other-conditions': AppState.userData.otherConditions
  };
...
function exportToCSV(weekPlan) {
  ...
  csvContent += `"Morning","PT Exercises","Physical Therapy"\n`;
...
function getAllUserConditions() {
  ...
  // Add primary diagnosis (legacy) - only if it's a valid condition ID
  if (AppState.userData.diagnosis && AppState.userData.diagnosis.trim() !== '' &&
      AppState.userData.diagnosis !== 'other') {
  ...
  if (AppState.userData.primaryConditions) {
    AppState.userData.primaryConditions.forEach(pc => {
      ...
      if (pc.id && pc.id.trim() !== '' && pc.id !== 'other' && !conditions.includes(pc.id)) {
...
function handleLogin(event) {
  ...
  if (result.hasData && AppState.hasCompletedIntake) {
    ...
    // New user or no data - go to intake
    navigateTo('intake');
  }
}
...
function saveState() {
  // Call original saveState for localStorage
...
function updateWeeklyProgressBars() {
  ...
  bars.forEach((bar, index) => {
    ...
    const dayOffset = (index - todayDay + 7) % 7;
    ...
    } else if (dayOffset < 0 || (dayOffset === 0 && index < todayDay)) {
      // Past days - show historical or simulated data
      const dateKey = getDateKey(new Date(today.getTime() - (todayDay - index) * 24 * 60 * 60 * 1000));
      const historical = progressHistory[dateKey] || Math.floor(Math.random() * 40 + 50);
...
function updateTaskBreakdown() {
  ...
  // Get actual completed tasks from AppState
  ...
  const homeCompleted = AppState.completedTasks?.home?.length || 0;

  // Estimate total tasks per category (based on typical daily tasks)
  const diagnosisDate = AppState.userData?.diagnosisDate ? new Date(AppState.userData.diagnosisDate) : new Date();
  const today = new Date();
  const daysSinceDiagnosis = Math.max(1, Math.floor((today - diagnosisDate) / (1000 * 60 * 60 * 24)) + 1);

  // Calculate expected tasks based on days in program
  const ptExpected = Math.max(ptCompleted, daysSinceDiagnosis * 3); // ~3 PT exercises per day
  const nutritionExpected = Math.max(nutritionCompleted, daysSinceDiagnosis * 3); // 3 meals per day
  const medExpected = Math.max(medCompleted, daysSinceDiagnosis * 2); // 2 medication times per day
  const wellnessExpected = Math.max(wellnessCompleted, daysSinceDiagnosis); // 1 wellness activity per day

  // Calculate percentages
  const ptPercent = ptExpected > 0 ? Math.round((ptCompleted / ptExpected) * 100) : 0;
  const nutritionPercent = nutritionExpected > 0 ? Math.round((nutritionCompleted / nutritionExpected) * 100) : 0;
  const medPercent = medExpected > 0 ? Math.round((medCompleted / medExpected) * 100) : 0;
  const wellnessPercent = wellnessExpected > 0 ? Math.round((wellnessCompleted / wellnessExpected) * 100) : 0;

  breakdownList.innerHTML = `
    <div class="task-breakdown-item">
      <div class="task-breakdown-info">
        <span class="task-breakdown-name">Physical Therapy</span>
        <span class="task-breakdown-count">${ptCompleted}/${ptExpected} completed</span>
      </div>
    ...
    <div class="task-breakdown-item">
      <div class="task-breakdown-info">
        ...
        <span class="task-breakdown-count">${nutritionCompleted}/${nutritionExpected} completed</span>
      </div>
      <div class="task-breakdown-bar">
    ...
    <div class="task-breakdown-item">
      <div class="task-breakdown-info">
        ...
        <span class="task-breakdown-count">${medCompleted}/${medExpected} completed</span>
      </div>
      <div class="task-breakdown-bar">
    ...
    <div class="task-breakdown-item">
      <div class="task-breakdown-info">
        ...
        <span class="task-breakdown-count">${wellnessCompleted}/${wellnessExpected} completed</span>
      </div>
      <div class="task-breakdown-bar">
...
const TutorialSystem = {
  ...
  steps: [
    ...
    },
    {
      ...
      message: "Your daily tasks are organized into categories - Physical Therapy, Medications, Nutrition, and Wellness. Check off items as you complete them.",
      target: '.task-categories',
      position: 'top'
    },
    {
      title: "Navigate Your Care",
      message: "Use the navigation menu to explore Nutrition, Medication, Physical Therapy, and more. Each section is tailored to your specific needs.",
      target: '.nav-menu',
      position: 'right'
    },
    {
  ...
  start() {
    ...
    if (localStorage.getItem('caribou_tutorial_completed')) {
      return;
    }
  ...
  updateStep() {
    ...
    document.getElementById('tutorial-step-total').textContent = this.steps.length;
  ...
  positionBubble(step) {
    ...
    switch (step.position) {
      case 'bottom':
        bubble.style.top = (rect.bottom + arrowOffset) + 'px';
        bubble.style.left = Math.max(padding, Math.min(rect.left + rect.width/2 - bubbleWidth/2, window.innerWidth - bubbleWidth - padding)) + 'px';
        arrow.classList.add('top');
        break;
      case 'top':
        bubble.style.top = (rect.top - bubbleHeight - arrowOffset) + 'px';
        bubble.style.left = Math.max(padding, Math.min(rect.left + rect.width/2 - bubbleWidth/2, window.innerWidth - bubbleWidth - padding)) + 'px';
        arrow.classList.add('bottom');
        break;
      case 'left':
        bubble.style.top = Math.max(padding, rect.top + rect.height/2 - bubbleHeight/2) + 'px';
        bubble.style.left = Math.max(padding, rect.left - bubbleWidth - arrowOffset) + 'px';
        arrow.classList.add('right');
        break;
      case 'right':
        bubble.style.top = Math.max(padding, rect.top + rect.height/2 - bubbleHeight/2) + 'px';
        bubble.style.left = (rect.right + arrowOffset) + 'px';
